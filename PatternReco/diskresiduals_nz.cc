...
gStyle->SetOptTitle(1);
 ...
 ifstream in("diskresiduals_approxTrue.txt");
 ...
 while (in.good()) {
   ...
   double disk,isPS,pt,idphi,dphi,dphicut,idr,dr,drcut,rinv, ir, iphi, iz, z;
   int seedindex=-1;

   in>>disk>>isPS>>seedindex>>pt>>idphi>>dphi>>dphicut>>idr>>dr>>drcut>>rinv>>ir>>iphi>>iz>>z;
 ...
 c1->Print("diskresiduals_nz.pdf(","pdf"); 
